			...
			<table className="w-full border-collapse space-y-1">
				<tbody>
					{MONTHS.map((monthRow) => {
						return (
							<tr
								key={`row-${monthRow[0]?.number ?? "months"}`}
									...
									const isDisabled =
										(maxDate
											? menuYear > maxDate.getFullYear() ||
												(menuYear === maxDate.getFullYear() &&
													monthItem.number > maxDate.getMonth())
											: false) ||
										(minDate
											? menuYear < minDate.getFullYear() ||
												(menuYear === minDate.getFullYear() &&
													monthItem.number < minDate.getMonth())
											: false) ||
										(disabledDatesMapped
											? disabledDatesMapped.some(
													(d) => d.year === menuYear && d.month === monthItem.number,
												)
											: false);

									const isSelected = month === monthItem.number && menuYear === year;
									...
									return (
										...
										>
											<button
												type="button"
												onClick={() => {
													setMonth(monthItem.number);
													setYear(menuYear);
													if (onMonthSelect) {
														onMonthSelect(new Date(menuYear, monthItem.number));
													}
												}}
												disabled={isDisabled}
												className={cn(
